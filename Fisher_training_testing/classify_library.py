...
class_index_file = "/home/sanjay/Music/improved_trajectory_release/CS221_Project-master/class_index.npz"
...
def make_FV_matrix(videos, fisher_path, class_index):
    ...
    for video in videos:
        ...
        matrix.append(np.load(vid_path,allow_pickle=True)['fish'])
        name = string.lower(video.split('_')[1])
...
def limited_input(training_dict, testing_dict, C, T):
    ...
    print(np.shape(tkeys_overC))
    print(np.shape(training_dict.keys()))
    print(np.shape(testing_dict.keys()))
